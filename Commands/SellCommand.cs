...
public static class SellCommand
{
    public static void Execute(CommandLineApplication app)
    {
        ...
        var quantityArg = app.Argument<int>("quantity", "Number of shares to sell");
        ...
        app.OnExecute(() =>
        {
            ...
            try
            {
                ...
                Console.WriteLine($"[INFO] Placing sell order: {quantity} shares of {symbol}" + 
                    (limitPrice.HasValue ? $" @ ${limitPrice:F2}" : " at market price"));

                var result = broker.PlaceOrderAsync(symbol, quantity, OrderSide.Sell, orderType, limitPrice).GetAwaiter().GetResult();
